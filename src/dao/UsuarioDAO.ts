...
class UsuarioDAO {
  ...
  protected static async iniciarSesion(
    ...
    UsuarioEsquema.findOne({ correoUsuario: miCorreo })
      .populate({ path: "codPerfil", select: "nombrePerfil" })
      .exec((miError, objeto) => {
        if (objeto) {
          const claveCorrecta = cifrado.compareSync(
            miClave,
            objeto.claveUsuario
          );
          if (claveCorrecta) {
  ...
  protected static async crearUsuario(
    ...
    if (existe) {
      ...
      parametros.claveUsuario = cifrado.hashSync(parametros.claveUsuario, 8);
        ...
        (miError: any, objeto: any) => {
          if (miError) {
            ...
            res
              ...
              .json({
                respuesta:
                  "Error al actualizar el usuario, puede que el correo esté repetido",
              });
          } else {
